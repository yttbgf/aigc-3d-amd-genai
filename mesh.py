...
class Mesh:
    ...
    def __init__(
        ...
        '''
        初始化 Mesh 类实例的各个属性，包括顶点 (v)、面 (f)、顶点法线 (vn)、面法线 (fn)、纹理坐标 (vt) 和面纹理坐标 (ft)。

此外，这段代码还包括了可选的属性，如 albedo（用于纹理映射）和 vc（用于顶点颜色）。此外，代码中还包含一个 device 参数，用于指定存储张量的设备（CPU 或 GPU）。

在 __init__ 方法中，还定义了 ori_center 和 ori_scale 属性来存储在对网格应用任何变换之前的原始中心和比例。在初始时，这两个属性被设置为 0 和 1，分别表示中心和比例。
Albedo 是一个西班牙语单词，来自拉丁语 "albedo"，意思是 "白色的" 或 "反射的"。在计算机图形学和物理学中，它被用来描述物体表面的反射率或反射系数。

在计算机图形学中，Albedo 通常用作纹理贴图的颜色值。在 3D 模型中，Albedo 贴图表示物体表面的反射属性。Albedo 贴图中的每一个像素都包含一个 RGB 值，用来表示该点的反射率。
    ...
    def load_obj(cls, path, albedo_path=None, device=None):
        ...
        # device
        if device is None:
            device = torch.device(common.global_constant.GPU.DEVICE if torch.cuda.is_available() else "cpu")
