	...
	const bool debug)
{
  ...
  printf("P:%d, H:%d, W:%d \n", P, H, W);
  //std::cout << " int_opts:" << int_opts << " float_opts:" << float_opts << std::endl;
  //printf("P:%d, H:%d, W:%d, int_opts:%d, float_opts:%d\n", P, H, W, int(int_opts), int(float_opts));
  ...
  //byte buffer & bind tensor data ptr
