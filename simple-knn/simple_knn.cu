...
void SimpleKNN::knn(int P, float3* points, float* meanDists)
{
	...
	coord2Morton <<<(P + 255) / 256, 256 >>> (P, points, minn, maxx, morton.data().get());
	//coord2Morton <<< (P + 255) / 256, 256, 0, hipStreamDefault >>> (P, points, minn, maxx, morton.data().get());
	//hipLaunchKernelGGL(coord2Morton, dim3((P + 255) / 256), dim3(256), 0, 0, P, points, minn, maxx, morton.data().get());
	...
	boxMinMax <<<num_boxes, BOX_SIZE >>> (P, points, indices_sorted.data().get(), boxes.data().get());
	boxMeanDist <<<num_boxes, BOX_SIZE >>> (P, points, indices_sorted.data().get(), boxes.data().get(), meanDists);
	//boxMinMax <<<num_boxes, BOX_SIZE, 0, hipStreamDefault >>> (P, points, indices_sorted.data().get(), boxes.data().get());
	//boxMeanDist <<<num_boxes, BOX_SIZE, 0, hipStreamDefault >>> (P, points, indices_sorted.data().get(), boxes.data().get(), meanDists);
	//hipLaunchKernelGGL(boxMinMax, dim3(num_boxes), dim3(BOX_SIZE), 0, 0, P, points, indices_sorted.data().get(), boxes.data().get());
	//hipLaunchKernelGGL(boxMeanDist, dim3(num_boxes), dim3(BOX_SIZE), 0, 0, P, points, indices_sorted.data().get(), boxes.data().get(), meanDists);
	cudaFree(result);
}