...
class GUI:
    def __init__(self, opt):
        ...
        self.input_img_torch_channel_last_list = []
    ...
    def prepare_train(self):
        ...
        if self.opt.input_camera_pose is not None:
            ...
            for pose in input_camera_poses:
                ...
                self.fixed_cam_list.append((pose, self.cam.perspective))
        else:
        ...
        for img_idx in range(len(self.input_img_list)):
            ...
            self.input_img_torch_channel_last_list.append(input_img_torch[0].permute(1, 2, 0).contiguous())
        ...
        for step in range(self.train_steps):  # 1??
            ...
            if not self.opt.imagedream:  # NOTICE: imagedream not use as ground truth?
                ...
                for idx_image in range(len(self.input_img_torch_list)):
                    ...
                    out = self.renderer.render(*cur_cam, self.opt.ref_size, self.opt.ref_size, ssaa=ssaa)

                    # rgb loss
                    image = out["image"]  # [H, W, 3] in [0, 1]
                    valid_mask = ((out["alpha"] > 0) & (out["viewcos"] > 0.5)).detach()
                    mse_loss_rgb = F.mse_loss(image * valid_mask, self.input_img_torch_channel_last_list[idx_image] * valid_mask)
                    print('mse_loss_rgb:', mse_loss_rgb)
                    loss = loss + mse_loss_rgb
            ...
            for batch_i in range(self.opt.batch_size):
                ...
                if self.opt.mvdream or self.opt.imagedream:  # other views: rotate horizontally based random view
                    ...
                    for view_i in range(1, 4):  # 3 views each rotate 90deg
                        ...
                        out_i = self.renderer.render(pose_i, self.cam.perspective, render_resolution, render_resolution,
                                                     ssaa=ssaa)
            ...
            if self.enable_sd:
                ...
                if self.opt.mvdream or self.opt.imagedream:
                    ...
                    refined_images = F.interpolate(refined_images, (render_resolution, render_resolution),
                                                   mode="bilinear", align_corners=False)
                    ...
                    refined_images = F.interpolate(refined_images, (render_resolution, render_resolution),
                                                   mode="bilinear", align_corners=False)
                    sd_loss = F.mse_loss(images, refined_images)
                    print('sd_loss:', sd_loss)
                    loss = loss + self.opt.lambda_sd * sd_loss
            ...
            if self.enable_zero123:
                ...
                refined_images = self.guidance_zero123.refine(images, vers, hors, radii, strength=strength,
                                                              default_elevation=self.opt.elevation).float()
                refined_images = F.interpolate(refined_images, (render_resolution, render_resolution), mode="bilinear",
                                               align_corners=False)
                sd_loss = F.mse_loss(images, refined_images)
                print('sd_loss:', sd_loss)
                loss = loss + self.opt.lambda_zero123 * sd_loss
