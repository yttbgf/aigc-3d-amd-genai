...
class Renderer(nn.Module):
    def __init__(self, opt):
        ...
        self.mesh = Mesh.load(self.opt.mesh, resize=False)  # No trainable parameters or NN Module
    ...
    def render(self, pose, proj, h0, w0, ssaa=1, bg_color=1, texture_filter='linear-mipmap-linear'):
        '''
        v, albedo has grad! check process which tensor has grad
        Args:
            pose:
            proj:
            h0:
        ...
        if self.opt.train_geo:
            ...
            v = self.mesh.v
        #print('v:', v)
        #print('v:', v.shape, v.grad is None)
        ...
        v_clip = v_cam @ proj.T
        #must use same gpu for training backward? cannot use subprocess or another file!!
        rast, rast_db = dr.rasterize(self.glctx, v_clip, self.mesh.f, (h, w))
        #print('rast:', rast)
        #print('rast_db:', rast_db)
        #print('rast:', rast.shape, rast.grad is None)
        #print('rast_db:', rast_db.shape, rast_db.grad is None)
        ...
        #print('albedo:', albedo.shape, albedo.grad is None)
        ...
        #print('alpha:', alpha.shape, alpha.grad is None)
        #print('normal:', normal.shape, normal.grad is None)
        #print('depth:', depth.shape, depth.grad is None)
        #print('viewcos:', viewcos.shape, viewcos.grad is None)
